...
def train(input_tensor, target_tensor, model,
    ...
    encoder_outputs = torch.zeros(
        max_length, model.encoder.hidden_size, device=model_config.device)
    ...
    decoder_input = torch.tensor(
        [[model_config.SOS_token]], device=model_config.device)
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            ...
            loss += criterion(decoder_output, target_tensor[di])
            if decoder_input.item() == model_config.EOS_token:
                break
...
def trainIters(model, pairs, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    start = time.time()
    plot_losses = []
